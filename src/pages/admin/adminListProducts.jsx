...
const AdminListProducts = () => {
    ...
    const fetch = async () => {
      ...
      setInitialData(() =>
        res.data.filter((item) =>
          item.detail_product.designation
            .toLowerCase()
      ...
      if (itemToDelete) {
        try {
          ...
          setStockData((prevData) =>
            prevData.filter((data) => data !== itemToDelete)
          );
  ...
  const handleProductUpdated = (updatedProduct) => {
    setStockData((prevData) =>
      prevData.map((item) =>
        item.pk === updatedProduct.pk ? updatedProduct : item
      )
    );
  };
  ...
  const onSubmitEdit = async (data) => {
    ...
    const updatedStockData = stockData.map((item) =>
      item.detail_product.designation ===
      selectedItem.detail_product.designation
        ? { ...item, ...data }
        : item
    );
